...
политика конфиденциальности titty.tools проста: мы ничего не собираем и не храним о
тебе. то, что ты делаешь, это исключительно твоё дело, а не наше или чьё-либо
ещё.

эти условия применяются только при использовании официального инстанса titty.tools.
...
при использовании функции сохранения, titty.tools может понадобиться проксировать
...
на неизменённом и официальном инстансе titty.tools **все данные туннеля шифруются
...
если исходный код titty.tools не изменён.

медиаданные из туннелей нигде не хранятся/кэшируются. всё обрабатывается в
реальном времени, даже при ремуксинге и транскодировании. туннели titty.tools
работают как анонимный прокси.

чтобы было понятно, как работает сохранение: медиафайлы вообще никогда не
записываются на диск сервера. извлечение лишь получает метаданные и прямые
ссылки на медиа из источника, а сами файлы затем стримятся напрямую с источника
на твоё устройство, как через прокси. единственные файлы, которые когда-либо
создаются на сервере, это файлы куки на каждый запрос, и они удаляются сразу
после извлечения, даже если скачивание не удалось.
...
github](https://github.com/bitceil/titty.tools/tree/main/api/src/stream), чтобы
...
число активных пользователей titty.tools, полностью анонимно. никакая
...
plausible](https://{env.PLAUSIBLE_HOST}/), который использует titty.tools.
